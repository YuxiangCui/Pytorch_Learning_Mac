...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.net = nn.Sequential(
            # z:[b, 2] => [b, 2]  前面的2由输入数据决定，后面由输出维度决定，这里要画图所以是2维
            nn.Linear(2, h_dim),
            nn.ReLU(True),
...
def data_generator():  # 这里设定一个已知的分布，8个高斯分布混合，来测试GAN是否可以学习得到
...
def main():
    ...
    for epoch in range(50000):
        ...
        # 1. train discriminator for 5 steps
        for _ in range(5):
            x = next(data_iter)
            ...
            # [b, 2] => [b, 1]
            predr = (D(xr))
            # max log(lossr) 使得预测数据来自真实数据的概率增大，加负号，minimize，可以使用梯度下降
            lossr = - (predr.mean())
            # fake data
            # [b, 2]
            ...
            # stop gradient on G
            # [b, 2]
            xf = G(z).detach()  # 这个地方停止计算梯度，不计算G的
            # [b]
            predf = (D(xf))
            # min predf
            lossf = (predf.mean())
